...
// Getting all users
router.get('/', (req, res) => {
  User.find({ age: { $lt: 33 } }) // This is means WHERE NAME == Hamdon && AGE == 24
  .limit(20)  //  This for setting a limit to the requesr
 ...
 * {
    "password": "1234",
    "email": "hussam.i1986@gmail.com"
}

 tryed on the saime path of hamdon mongodb 
 */
  User.findOne({
    email: req.body.email
  }, function(err, User) {
    ...
    if (!User) {
      ...
      if (! bcrypt.compareSync(req.body.password, User.password)) {
        ...
        return res.json({token: jwt.sign({ email: User.email, name: User.name, _id: User._id}, 'husam login API')});
      }
    }
  });
});
